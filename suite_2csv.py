...
def get_xlsx(file_path, project_name='ISON'):
    ...
    for i, mf in enumerate(meta_file_list):
        ...
        for num in range(object_num):
            z_index    = json_label_data['objects'][num]['annotation']['meta']['z_index']
            ...
            if 'class'+str(z_index) not in df.columns:
                df['class'+str(z_index)] = ''
                df['property'+str(z_index)] = ''
                print(f'{z_index}번 클래스 추가')
            
            df.loc[i]['class'+str(z_index)] = class_name
            df.loc[i]['property'+str(z_index)] = properties
